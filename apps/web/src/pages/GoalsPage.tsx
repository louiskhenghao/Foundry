            ...
            {goals.map((g) => (
              <Link key={g.id} to={href(g)} className="block rounded-lg border border-zinc-800 bg-zinc-900/40 p-3 hover:border-zinc-600">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge state={g.state} />{g.mode === 'simple' && <span className="text-[10px] rounded-full border border-zinc-700 text-zinc-400 px-1.5">simple</span>}{g.nature !== 'auto' && g.nature !== 'code' && <span className="text-[10px] rounded-full border border-sky-800 text-sky-300 px-1.5">{g.nature}</span>}
                </div>
