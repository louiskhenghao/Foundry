...
import { ChevronDown, ChevronRight, ListChecks, Plus, Sparkles, Trash2 } from 'lucide-react';
...
export function TaskCard({ task, brief, goalId, editable, open, onToggle, edit }: { task: BriefTask; brief: Brief; goalId: string; editable: boolean; open: boolean; onToggle: () => void; edit: (fn: (b: Brief) => Brief) => void }) {
  ...
  const remove = () => edit((b) => ({ ...b, tasks: b.tasks.filter((t) => t.key !== task.key).map((t) => ({ ...t, dependsOnKeys: t.dependsOnKeys.filter((k) => k !== task.key) })), checks: b.checks.filter((c) => c.taskKey !== task.key) }));
  ...
  return (
    <div className={cn('rounded-md border bg-zinc-950/50', problem && editable ? 'border-rose-500/40' : 'border-zinc-800')}>
      <div className="flex items-center gap-2 flex-wrap p-2.5">
        ...
        {editable && (
          <Button size="sm" variant="ghost" onClick={remove} title="Remove task (and its checks)">
            <Trash2 size={13} />
          </Button>
        )}
      </div>
      <div className="px-2.5 pb-2.5 flex flex-wrap items-center gap-2 text-xs">
        <Menu
          width="w-72"
          align="left"
          trigger={({ toggle }) => (
        ...
        {problem && editable && <span className="text-rose-300">{problem}</span>}
      </div>
