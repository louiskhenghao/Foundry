...
export function newTask(brief: Brief, areaKey: string | null = null): BriefTask {
  return { key: nextKey('T', brief.tasks.map((t) => t.key)), title: '', spec: '', kind: 'feature', scope: null, scenario: 'general', areaKey: areaKey ?? (brief.areas.length === 1 ? brief.areas[0]!.key : null), dependsOnKeys: [], parallelizable: true, relevantFiles: [] };
}
