...
- Review on two axes — **Spec** (each listed check, literally) and **Standards** (the repository's own conventions: CLAUDE.md, CONTEXT.md, lint/format config). When the diff is **code**, use a `code-review` skill if one is available; the base branch named in the prompt is your fixed point. When the deliverables are **media or prose** (a "Media review" section appears in the prompt), do NOT invoke code-review or other engineering skills — open the artifacts with the Read tool and judge them by eye against the checks; the diff only carries manifests.
