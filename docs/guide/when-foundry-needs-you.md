...
Once you approve the Brief, Foundry works on its own. It stops and asks you only in the situations on this page. Each one shows up in the **Inbox** (the number in the top bar), on the goal's page, and — if you set it up — as a Telegram or Discord message.
...
## The six reasons Foundry stops

### A question only you can answer

**Where:** the Brief page, before the goal starts.
...
- **Skip task** — move on without it. Tasks that depended on it still run; the final review judges the whole.

The last attempt of a task, and every retry you grant, run on the preset's strongest task model.

### Two pieces changed the same lines

**Inbox:** *Retries exhausted · merge conflict* — lists the files and why each merge attempt failed.

Two tasks edited the same code and Foundry could not combine them automatically. Press **Resolve manually →** to see both sides of each file, take one side, edit the result or open it in your editor, then **Finish merge**. You can also *Retry with hint* ("keep both, the new field goes after the old one") or *Skip task*.

### The final review failed

**Inbox:** *Retries exhausted · goal review* — lists the failing checks and the reviewer's notes.
...
- **Accept as-is** finishes the goal with those checks waived.

### It wants to do something outside your computer

**Inbox:** *Wants to leave the workspace*.

A session tried to push, deploy, or call a paid service. Foundry blocked it. **Approve & run once** runs that one command for it; **Deny** keeps it blocked. (Pushing and opening pull requests are normally done by Foundry itself at delivery, following the policy you chose — see [Getting the result](./getting-the-result.md).)
...
**Goal page:** *Have a look* — what to look at, and a link to the running preview.
...
A milestone pauses at most twice. After the second look, anything you write becomes a hint.
...
A task moves through **pending** (waiting for the tasks it depends on) → **ready** → **running** → **observing** (checks and review) → **merging** (joining the goal's branch) → **done**.
...
| *running · "retry 2/3"* | A normal new attempt after a failed one. |
