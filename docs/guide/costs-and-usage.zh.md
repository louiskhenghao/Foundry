...
Foundry 用的是你的 Claude 订阅或 API 账户。只有在 Claude 会话运行时才花钱。它在 git 里做的事、跑你的测试、准备文件夹、查看检查结果都是免费的 —— 只花时间，不花钱。

## 哪些要花钱

| 会话 | 每个会话的上限 |
|---|---|
| Clarify（探索你的仓库、访谈、写 Brief） | 每轮 $6 |
| Draft with AI（一个任务或一个 Area） | $2 |
| Revise with answers | $3 |
| 任务尝试 | 默认 $10（Settings → Models & limits），且不超过 goal 剩余的预算 |
| 任务审查 | $0.80 |
| 合并尝试（合并两个改了同一段代码的任务） | $2 |
| 最终 goal 审查 | 至少 $6，最多到任务尝试的上限 |
| 完成文档（仅在你开启时） | $3 |
...
- **Usage**（顶栏）按时间、会话类型和模型显示花费，以及离套餐上限还有多远。
