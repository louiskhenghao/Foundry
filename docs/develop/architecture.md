...
**When a delivery stops** (`delivery/inbox.ts`): every `delivery.failed` raises one `delivery_failed` Escalation with the reason; a new run (`delivery.started`) or a finished delivery closes it. Failing checks are recorded per PR with their description and link (`failingChecks` in `delivery/gh.ts`, `delivery.checks.failing`). `settlePr` hands a failure to a fix-CI task only when `failedLog` returns a CI log; a fix that commits nothing stops the delivery instead of re-pushing the same SHA. `Engine.retryDelivery` (policy_set with source `retry`, which resets `fixCycles`), `Engine.recheckDeliveryPr` and `Engine.markDelivered` (`delivery/pr-watch.ts`) back the Retry delivery, Re-check and Mark as delivered buttons (`POST /api/goals/:id/delivery/retry`, `…/prs/:n/recheck`, `…/mark-delivered`). The PR watch also covers failed deliveries with an open PR, so a PR merged by hand finishes the delivery.

**Commit identity** (`git/git.ts`): every engine commit, merge and cherry-pick uses `gitIdent(cwd)`, resolved per repository from Settings → Git & delivery → Commit author (`setCommitAuthorMode`): the repository's or global `user.name`/`user.email`, else the gh account's noreply address, else `foundry <foundry@local>`; `withCoauthor` adds `Co-authored-by: Foundry <foundry@local>` in the default mode. Stacked PR commits rebuilt at delivery are re-authored the same way.
...
**Skills operations.** Each Skills page action that installs, updates or removes something is a Skills operation (`SkillOps`, `packages/server/src/skill-ops.ts`): the route registers it, streams every line to `skills-op:<id>` and ends it `ok` or `failed` with a one-line summary, which is also the channel's last line (`✔`/`✘`). The page picks the id (`opId` in the body) so it can follow the channel before the request answers; callers without one (the CLI) get a server id. Installs, tier installs, adoptions and bulk uninstalls answer when they finish, with their usual body plus `op` (an error body carries `op` too); source updates and tool installs answer `202` and run on. The engine's own rules still apply: calls queue on the skills manager's serial chain, and a second source update is refused with `409` before any operation is registered. Tool installs without an `opId` also mirror their lines to `tool-install`. The registry lives in memory (the last 50 finished operations, 1000 lines each), and `/api/stream/skills-op:<id>/history` replays its lines, so a reload refills the log; a server restart forgets them. On the page, `pages/skills/ops.ts` keeps one tab per operation in a zustand store saved to sessionStorage, takes the status from the request's answer or by polling `/api/skills/ops/:id` (a 404 marks the tab lost), and adds running operations it did not start from `/api/skills/ops`; `OpsDock.tsx` shows them docked at the bottom of the page.
