...
Run from the Foundry checkout (`bun apps/cli/src/main.ts <command>`, or `bun run cli <command>`). Commands other than `serve`, `doctor` and `replay` talk to a running server on `FOUNDRY_URL` (default `http://127.0.0.1:4111`).
