...
| **Interview round N** — on the goal page while the goal is *clarifying* | The Clarifier asked what only you can decide before writing the Brief; nothing runs until you answer. | Pick an option per question (the first is its recommendation) or type your own; **Accept all recommended** answers a round in one click; **Enough — write the Brief** stops the questions. Blocking questions need an answer unless you press Enough. |
| **Have a look** — *"&lt;task&gt; landed. &lt;what to look at&gt;"* + a preview link | A milestone task landed; the goal waits for you (state *have a look*). Nothing else starts meanwhile. | Open the goal page: the preview (start/stop, open), the self-check's screenshots and the artifacts are there. **Continue**, or write what you saw → **Turn into a plan** → confirm: a hint for the remaining tasks, fix tasks (then a second look at the same milestone), or a Decision every later task follows. |
...
| *[preview] … did not answer within 90s — the server may still be starting* | The run command started but nothing listened on the port in time (slow install, wrong port flag). | Check the server output on the Preview card; set the Brief's *How to run it* command (use `{port}`) when detection guessed wrong. |
| *self-check could not run: nothing to run / Playwright is not installed / Chromium could not start* | The goal has the self-check on but there is no run command, or the browser is missing. | Settings → Preview → *Install Chromium*; or set the run command in the Brief; or switch the goal's self-check off on its Preview card. The check counts as `error`, not `fail`. |
