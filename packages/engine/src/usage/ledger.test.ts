...
describe('usage dashboard aggregates', () => {
  test('series buckets cover the whole window with empty buckets; totals and byGoal titles', async () => {
    ...
    s.append({ type: 'goal.created', goalId: 'g_t', payload: { goal: { id: 'g_t', title: 'Titled goal', prompt: 'p', repoPath: '/r', baseBranch: 'main', branch: 'goal/g_t', budgets: { maxCostUsd: 5, maxDurationMin: 120, maxConcurrent: 3, attemptsPerTask: 3 }, budgetPreset: 'custom', models: { strong: 'opus', cheap: 'haiku', worker: 'opus' }, state: 'done', stateBeforeBlock: null, costUsd: 1, fixCycles: 0, delivery: IDLE_DELIVERY, attachments: [], baseSync: null, autoskills: null, runningSince: null, createdAt: now, updatedAt: now } } });
