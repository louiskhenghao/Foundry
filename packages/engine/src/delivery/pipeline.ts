...
function syntheticMergeTask(ctx: Ctx, ref: string, into: string): Task {
  ...
  const task: Task = {
    ...
    retryBudget: 2,
    origin: 'merge', milestone: null, milestoneVisits: 0, checkpointOf: null, difficulty: 'normal' as const,
    state: 'merging',
    branch: null,
    worktreePath: null,
    baseRef: null,
...
async function fixCi(ctx: Ctx, repo: string, prNumber: number, unit: PrUnit): Promise<boolean> {
  ...
  const task: Task = {
    ...
    origin: 'delivery-fix', milestone: null, milestoneVisits: 0, checkpointOf: null, difficulty: 'normal' as const,
    state: 'running',
    branch: null,
    worktreePath: null,
    baseRef: base,
