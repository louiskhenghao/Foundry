...
export function applySettingsToConfig(config: EngineConfig, s: Settings, only?: Set<string>): void {
  ...
  for (const role of ['clarifier', 'planner', 'merger', 'goalReviewer', 'taskReviewer', 'documenter', 'feedback', 'suggest', 'styleSample'] as const) if (on(`models.${role}`)) config.modelRoles[role] = s.models[role];
