...
import { IdPrefix, bumpStat, getBrief, getObservation, listAttempts, listChecks, newId, renderDecisions } from '@ai-engine/core';
...
export async function runAttempt(engine: Engine, goal: Goal, task: Task, cwd: string, opts: { baseMoved?: CatchUp | null } = {}): Promise<AttemptOutcome> {
  ...
  const taskBaseRef = prior.find((a) => a.baseRef)?.baseRef ?? baseRef;
  const prevReport = prior.length ? getObservation(store.db, prior[prior.length - 1]!.id) : null;
  const now = new Date().toISOString();
  const attemptId = newId(IdPrefix.attempt);
  ...
  const handle = await engine.runner.run({
    ...
    label: `attempt ${task.title} #${index}`,
  });
  ...
  return { attempt, report, result, passed: allMustPassed, nonBoundaryDenials };
}
